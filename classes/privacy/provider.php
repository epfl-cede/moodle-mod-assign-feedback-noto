...
namespace assignfeedback_noto\privacy;
...
class provider implements
    ...
    public static function export_feedback_user_data(assign_plugin_request_data $exportdata) {
        ...
        $currentpath[] = get_string('privacy:path', 'assignfeedback_noto');
        $assign = $exportdata->get_assign();
        $plugin = $assign->get_plugin_by_type('assignfeedback', 'noto');
        ...
        if ($filefeedback) {
            ...
            foreach ($fileareas as $filearea => $notused) {
                \core_privacy\local\request\writer::with_context($exportdata->get_context())
                        ->export_area_files($currentpath, 'assignfeedback_noto', $filearea, $gradeid);
            }
        }
    }
    ...
    public static function delete_feedback_for_context(assign_plugin_request_data $requestdata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            ...
            $fs->delete_area_files($requestdata->get_context()->id, 'assignfeedback_noto', $filearea);
        }
    ...
    public static function delete_feedback_for_grades(assign_plugin_request_data $deletedata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete feedback files.
            $fs->delete_area_files_select($deletedata->get_context()->id, 'assignfeedback_noto', $filearea, $sql, $params);
        }

        // Delete table entries.
        $DB->delete_records_select('assignfeedback_noto', "assignment = :assignment AND grade $sql", $params);
    }
}
